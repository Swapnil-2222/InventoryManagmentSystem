...
public interface TransferRepository extends JpaRepository<Transfer, Long>, JpaSpecificationExecutor<Transfer> {
    ...
    @Query(
        value = "select distinct transfer from Transfer transfer left join fetch transfer.securityUser",
    ...
    @Query("select distinct transfer from Transfer transfer left join fetch transfer.securityUser")
    List<Transfer> findAllWithToOneRelationships();

    @Query("select transfer from Transfer transfer left join fetch transfer.securityUser where transfer.id =:id")
