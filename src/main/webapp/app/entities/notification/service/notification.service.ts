...
export class NotificationService {
  ...
  protected convertDateFromClient(notification: INotification): INotification {
    return Object.assign({}, notification, {
      lastModified: notification.lastModified?.isValid() ? notification.lastModified.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((notification: INotification) => {
        notification.lastModified = notification.lastModified ? dayjs(notification.lastModified) : undefined;
      });
    }
    return res;
  }
